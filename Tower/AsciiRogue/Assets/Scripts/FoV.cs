...
public class FoV 
{
    ...
    {
        for (int y = 0; y < DungeonGenerator.dungeonGenerator.mapHeight; y++)
        {
            for (int x = 0; x < DungeonGenerator.dungeonGenerator.mapWidth; x++)
            {
    ...
    /*static public void VisibilityCheck(Vector2Int position)
    {
        ...
        {
            MapManager.map[position.x, position.y].color = "#ffffff";
        }
        else
        {
            if (MapManager.map[position.x, position.y].isExplored)
            {
                MapManager.map[position.x, position.y].color = "#808080";
            }
            else
            {
                MapManager.map[position.x, position.y].color = "#000000";
            }
        }
    }*/
    ...
    static List<Vector2Int> GetCellsAlongLine(Vector2Int origin, Vector2Int destination)
    {
        ...
        int mapWidth = DungeonGenerator.dungeonGenerator.mapWidth;
        int mapHeight = DungeonGenerator.dungeonGenerator.mapHeight;
