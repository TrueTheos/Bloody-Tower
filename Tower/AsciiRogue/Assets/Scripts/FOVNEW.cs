...
public class FOVNEW : MonoBehaviour
{
    ...
    public int TorchDistance(int x, int y, Vector2Int origin)
    {
        return (int)Mathf.Max(Mathf.Abs(x - origin.x), Mathf.Abs(y - origin.y));
    }
    ...
    void ComputeTorch(int octant, Vector2Int origin, int rangeLimit, int x, Slope top, Slope bottom)
    {
        for (; (int)x <= (int)rangeLimit; x++) // rangeLimit < 0 || x <= rangeLimit
        {
            ...
            for (int y = topY; y >= bottomY; y--)
            {
                ...
                bool inRange = rangeLimit < 0 || TorchDistance(tx, ty, origin) <= rangeLimit;
